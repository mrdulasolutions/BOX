...
- The plugin works the same on Claude Code desktop, Claude.ai/code, and (when the same MCP is available elsewhere) any other Claude-compatible client.
...
|---|---|---|
| Box MCP not connected | Tool calls fail | Skill instructions tell agent to surface a clear message: "Connect Box MCP via Claude settings." |
